  ...
  def amount_per_day
    ...
    if project_donations.blank? and project_sponsors.blank?
      ...
      self.created_at.to_date.upto(Time.now.to_date).each do |date|
        ...
        amount += project_sponsors.select{ |sponsor| sponsor.updated_at > date.at_beginning_of_day and sponsor.updated_at < date.end_of_day }.map { |sponsor| sponsor.cost }.compact.inject(0) {|sum, element| sum + element }
