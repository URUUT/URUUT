class PagesController < ApplicationController
  ...
  def projects_list
    ...
    @projects = Project.where("#{query.join(" OR ")} AND live = ? AND status IS NULL OR status = ? AND ready_for_approval = 0", :keyword=> "%#{params[:keyword]}%, 1, ''").by_city(params[:city]).by_category(params[:category])
  end
