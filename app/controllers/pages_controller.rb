class PagesController < ApplicationController
  ...
  def projects_list
    ...
    @projects = Project.where("#{query.join(" OR ")} AND live = 1 AND status IS NULL OR status = nil AND ready_for_approval = 0", :keyword=> "%#{params[:keyword]}%").by_city(params[:city]).by_category(params[:category])
  end
